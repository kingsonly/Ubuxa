...
.form-wrapper .button, .form-wrapper .client-submit, .form-wrapper .submit{
  background-color: #3498db;
  display: inline-block;
  padding: 8px 30px;
  color: #fff;
  ...
  <div class="client-container">
    <div class="wrapper-form">
      ...
      <?php $form = ActiveForm::begin(['options' => [
                    'class' => 'form-wrapper client-form'
                 ]
         ]); ?>
      ...
      <?= $form->field($addressModel, 'state_id')->widget(DepDrop::classname(), [
         ...
         'pluginOptions'=>[
          ...
          <div class="button_prev">Previous</div>
          <?= Html::submitButton('Finish', ['class' => 'btn btn-success client-submit']) ?>
        </fieldset>
        <fieldset class="section">
...
<div class="button-select dropdown"> 
  <select id="colorselector" class="colorselectorclient">
...
<div class="output">
  ...
  <div  class="error_client" style="color:red"></div>
</div>
...
<?php 
$clientUrlSave = Url::to(['client/create']);
$clientUrlExisting = Url::to(['client/create','src' => 'existing']);
$clientFormJs = <<<JS
...
$('.client-form').submit(function(e){
  ...
  $('.form-clients').find('input, textarea').on('keyup blur focus', function (e) {
...
$(function() {
  $('.colorselectorclient').change(function(){
    var get_text = $(this);
    ...
    $.ajax({
                ...
                success: function(response) {
                  if(response.status == 1){
                      ...
                      $('.colors').hide();
                      $('.error_client').text();
                      $('#blue').html('you have added <strong>'+ corporation + '</strong> as a new client');
                      $('#blue').show();
                    }else {
